...
public class ImportFromFile {
    ...
    private static Entity[][] parseMap(String mapTxt) {
        ...
        Entity[][] entityMap = new Entity[position.length][position[0].length];
        for (int i = 0; i < position.length; i++) {
            for (int j = 0; j < position[0].length; j++) {
                int entity = Integer.parseInt(position[i][j]);
                switch (entity) {
                    case 0:
                        entityMap[i][j] = new CorridorTile(i, j);
                        break;
                    case 1:
                        entityMap[i][j] = new Wall(i, j);
                        break;
                    case 2:
                        entityMap[i][j] = new RoomTile(i, j);
                        break;
                    case 3:
                        entityMap[i][j] = new Door(i, j);
                        break;
                }
            }
        }
